...
var grabBlocks = function(config) {
    var web3 = new Web3(new Web3.providers.HttpProvider(config.gethAddr + ':' + config.gethPort.toString()));
...
if (!('gethAddr' in config) || (typeof config.gethAddr) !== 'string') {
    config.gethAddr = 'http://localhost'; // default
}

// set the default output directory if it's not provided
if (!('output' in config) || (typeof config.output) !== 'string') {
    config.output = 'http//localhost'; // default this directory
}
